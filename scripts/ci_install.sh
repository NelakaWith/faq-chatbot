...
if command -v npx >/dev/null 2>&1 && npx --no-install --quiet semantic-release --version >/dev/null 2>&1; then
    ...
    npx --no-install semantic-release --version || echo "Direct npx test failed"
    exit 1
fi
